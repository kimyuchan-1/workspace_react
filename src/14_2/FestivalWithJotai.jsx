...
import { selGuAtom, festivalFetchAtom } from './atomsFestival';
import { useAtom } from 'jotai';
import { Suspense } from 'react';

export default function FestivalWithJotai() {
    return (
...
function FestivalContent() {
    const [tdata] = useAtom(festivalFetchAtom);
    ...
    return (
        <div className="w-full h-full flex flex-col justify-start items-center">
            ...
            <div className="mt-4 w-9/10 h-3/4 overflow-y-auto
                ...
                {
                    areaFestival.map((item, idx) => <Link to="/festivalwithjotai/contents"
                        state={{ contents: item }}
                        key={item.UC_SEQ + idx}>
                        <TailCard key={item.UC_SEQ}
                            url={item.MAIN_IMG_THUMB}
                            title={item.MAIN_TITLE.includes('(') ? item.MAIN_TITLE.split('(')[0] : item.MAIN_TITLE}
                            location={item.TRFC_INFO}
                            keyword={item.ADDR1}
                        />
                    </Link>
                    )
